...
METHODS = [
    ...
    "shortest_queue",
    "least_work",
]  # Métodos testados no load balancer
...
class Metrics:
    ...
    def show_metrics(self):
        ...
        print(
            f"Utilizacao media do sistema (considerando o emergencial): {(sum_utilization / (NUM_SERVERS + 1)):.2f}%"
        )
        ...
        if self.discarded_requests > 0:
            ...
            print(
                f"Requisições Descartadas: {self.discarded_requests} ({(self.discarded_requests / (self.discarded_requests + len(self.response_times))) * 100:.2f}%)"
            )
        else:
            print(f"Requisições Processadas: {len(self.response_times)}")
    ...
    def distribute(self, req):
        ...
        # política que faz uso do fato dos servidores terem processamentos diferentes
            ...
            for i, server in enumerate(self.servers):
                ...
                current_workload = (
                    server.users[0].req["duration"] if server.users else 0
                )
                # tempo de todos na fila
                queue_workload = sum(
                    r.req["duration"] for r in server.queue if hasattr(r, "req")
                )
